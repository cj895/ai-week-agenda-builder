...
export default function AgendaBuilder(){
  ...
  const doExport=(filterDay,filterLoc)=>{
    ...
    const rows=data.sort((a,b)=>a.date.localeCompare(b.date)||a.startTime.localeCompare(b.startTime)||a.location.localeCompare(b.location)).map(s=>{const t=trk(s.trackId);const tags=[...(s.topicTags||[]),...(s.audienceTags||[])].join(",");const desc=[s.sponsor?.trim()?"Sponsored by "+s.sponsor+".":"",s.description||""].filter(Boolean).join(" ");return[s.date,t.name,s.title||"TBD",s.startTime,s.endTime,s.location,"",t.bg,t.fg,tags,(s.speakers||[]).join(","),s.sessionType,"No","Unlimited",desc,"","","","","","","","","","","","","","","","","",s.status].map(esc).join(",")});
    const csv="\uFEFF"+hdr.join(",")+"\n"+rows.join("\n");
